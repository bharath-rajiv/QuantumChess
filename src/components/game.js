  ...
  handleClick(i) {
    ...
    if (this.state.sourceSelection === -1) {
      if (!squares[i] || squares[i].player !== this.state.player) {
        this.setState({ status: "Not Your Turn. Player" + this.state.player + " move." });
        if (squares[i]) {
          squares[i].style = { ...squares[i].style, backgroundColor: "" };
        }
      }
      else {
        squares[i].style = { ...squares[i].style, backgroundColor: "RGB(255,192,203)" }; 
        this.setState({
          status: "Where Do you want to Place it ?",
          sourceSelection: i
        })
      }
      return
    ...
    if (squares[i] && squares[i].player === this.state.player) {
      this.setState({
        status: "Not a valid move dumb ass!",
        sourceSelection: -1,
      });
    }
    else {
